...
p = [0,0,0]
p[0] = array([0.5,0.5]) #position of charge
p[1] = array([0.25,0.5]) #position of charge
p[2] = array([0.75,0.75]) #position of charge
...
def main():
    ...
    for i in range(len(p)):
        nx = floor(p[i][0]/(box[1]/N))
        ny = floor(p[i][1]/(box[1]/N))
        field[nx][ny] = 1./(dx*dx)
